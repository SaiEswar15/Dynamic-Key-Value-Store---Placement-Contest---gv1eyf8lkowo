...
const KeyValueStore = () => {
    ...
    const handleUpdateClick = () => {
      const queryParams = Object.keys(keyValues).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(keyValues[key])}`).join('&');
...
{Object.entries(keyValues).map(([key,value])=>(
              <div key={key} className='key-value-div'>
                ...
                <input
                    ...
                    type="text" value = {value} onChange = {(e)=>updateKeyValue(key,e.target.value)}
                />
